...
namespace OOCL.Image.Client
{
	public class ApiClient
	{
		...
		{
			await this.logger.LogAsync($"Called ExecuteTimestretchAsync(id: {(id.HasValue ? id.Value.ToString() : "null")}, dto: {(dto != null ? $"provided ({dto.Data.Length} bytes)" : "null")}, {kernelName}, {chunkSize}, {overlap}, {factor})", nameof(ApiClient));
			try
			{
