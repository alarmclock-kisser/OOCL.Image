...
namespace OOCL.Image.OpenCl
{
	public class OpenClCompiler
	{
		...
		public string KernelPath { get; set; } = Path.GetFullPath(Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "..", "..", "..", "..", "OOCL.Image.OpenCl", "Kernels"));
		...
		public bool TryAnalog
		{
			...
			set
			{
				...
				this.Arguments = value ? this.GetArguments(true) : this.GetArguments();
			}
		}
		...
		public object[] GetArgumentDefaultValues()
		{
			...
			foreach (var arg in arguments)
			{
				...
				{
					argValues[i] = type == typeof(Vector2) ? Vector2.Zero : 0;
				}
				i++;
			}
			return argValues;
